...
void ICPRegistration::SetSourceCloud(const pcl::PointCloud<pcl::PointXYZI>::Ptr &source_cloud_ptr)
{
    ...
    this->_source_center_vec = std::accumulate(source_cloud_ptr->points.begin(), source_cloud_ptr->points.end(),
...
void ICPRegistration::SetTargetCloud(const pcl::PointCloud<pcl::PointXYZI>::Ptr &target_cloud_ptr)
{
    ...
    this->_target_center_vec =
        std::accumulate(
            target_cloud_ptr->points.begin(), target_cloud_ptr->points.end(),
            Eigen::Vector3d::Zero().eval(),[](const Eigen::Vector3d &sum, const pcl::PointXYZI &point) {
                return sum + point.getVector3fMap().cast<double>();
            }) /
...
void ICPRegistration::GetResTransform(Sophus::SE3d &init_transform)
{
    ...
    std::iota(index_map.begin(), index_map.end(),  _source_cloud_ptr->points.size()); // 源希腊字母  Ι ι aiot 约塔 微小，一点儿

    /**/
    // for (int iter = 0; iter < options_.max_iteration; ++iter)
    // {
    //     // std::for_each(std::execution::par_unseq, index_map.begin(), index_map.end(), []() {
