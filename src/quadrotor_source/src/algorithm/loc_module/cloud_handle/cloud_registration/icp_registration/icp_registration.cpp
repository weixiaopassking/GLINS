...
namespace algorithm_ns
{
    ...
    bool ICPRegistration::Point2Point(Sophus::SE3d & init_transform)
    {
        ...
        for (int iter = 0; iter < _options.max_iteration; ++iter)
        {
            ...
            int effect_cnt = 0;
            auto H_g = std::accumulate(
                index_map_vec.begin(), index_map_vec.end(), std::pair<H_type, g_type>(H_type::Zero(), g_type::Zero()),
                [&](const std::pair<H_type, g_type> &pre, int index) -> std::pair<H_type, g_type> {
                    ...
                    else
                    {
                        ...
                        effect_cnt++;
                        return std::pair<H_type, g_type>(pre.first +
                                                             jacobians_vec[index] * jacobians_vec[index].transpose(),
