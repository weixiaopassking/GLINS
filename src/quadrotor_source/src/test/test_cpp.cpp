...
TEST(example, instance1)
{
    ...
    RuntimeRecord([&num_table]() { std::sort(num_table[0].begin(), num_table[0].end()); }, "sort-none");

    RuntimeRecord([&num_table]() {
        std::sort(SEQ, num_table[1].begin(), num_table[1].end()); }, "sort-seq");
    RuntimeRecord([&num_table]() {
        std::sort(PAR, num_table[2].begin(), num_table[2].end()); }, "sort-par");
    RuntimeRecord([&num_table]() {
        std::sort(PAR_UNSEQ, num_table[3].begin(), num_table[3].end()); }, "sort-par_unseq");
}
