...
data_ns::Mat4f  ICPRegistration::Point2Point(const data_ns::Mat4f &predict_transform)
{
    ...
    for (int iter = 0; iter < _option.max_iteration; ++iter)
     {
        std::for_each(index_map_vec.begin(), index_map_vec.end(), [&](int index) {
            /*3.1--knn*/
            ...
            {
                Eigen::Vector3f p =
                    _target_cloud_ptr->points[nn_search_index_vec.front()].getVector3fMap();
                ...
                jacobian.block<3, 3>(0, 0) =
                    (res_se3_transform.so3().matrix() * Sophus::SO3f::hat(q)).transpose(); // derivative of R 3rows6cols
